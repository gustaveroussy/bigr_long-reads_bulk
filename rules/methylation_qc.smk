...
rule motif_cg_gref:
    ...
    shell:
        """
        {TOOL_MODKIT} motif-bed {input.ref_fa} CG 0 1> {output.cg_motif}
        """
...
rule concat_all_chromosome_per_sample:
    input:
        comb_bed = expand(os.path.normpath(OUTPUT_DIR + "/bed_combined_strands/{{sample_name}}/{{meth_type}}/{{sample_name}}_chr{chr_number}_{{meth_type}}_comb.bed"), chr_number = CHR_NUMBER),
    output:
        bed_comb_concat_formated = os.path.normpath(OUTPUT_DIR + "/bed_combined_strands/{sample_name}/{meth_type}/{sample_name}_{meth_type}_comb.bed")
    threads: 1
    resources:
        mem_mb = lambda wildcards, attempt: 1024 * attempt,
    ...
    shell:
        """
        sed "s/ /\t/g" {input.comb_bed} | awk  '{{print $1,$2,$3,$11,$12,$10-$12}}' > {output.bed_comb_concat_formated}
        """
...
rule meth_QC_barplot_CG:
    input:
        bed_comb_concat_formated_all_samples = expand(os.path.normpath(OUTPUT_DIR + "/bed_combined_strands/{sample_name}/{{meth_type}}/{sample_name}_{{meth_type}}_comb.bed"), sample_name = SAMPLE_NAME),
        cg_motif = os.path.normpath(OUTPUT_DIR + "/resources/motif_cg.bed"),
    output:
        os.path.normpath(OUTPUT_DIR + "/Quality_Control/methylation_QC/barplot_methylated_CG_{meth_type}_mqc.png")
    params:
        bed_comb_concat_formated_all_samples = params_bed_comb_concat_formated_all_samples,
        script = os.path.normpath(PIPELINE_DIR + "/script")
    threads: 2
    resources:
        mem_mb=lambda wildcards, attempt: min(5120 + 5120 * (attempt - 1 ),20480),
    ...
    shell:
        ...
        #module load r/3.6.1
        singularity exec --contain -B {params.script},{OUTPUT_DIR} {SING_ENV_GENEDMR} \
        Rscript {params.script}/QC_methylation_barplot_methylated_CG.R --list_input_bed_comb_concat {params.bed_comb_concat_formated_all_samples} --input_bed_motif_cg {input.cg_motif} --output_path {OUTPUT_DIR}/Quality_Control/methylation_QC/ 
        """
